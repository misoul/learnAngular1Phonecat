...
exports.registerRoutes = function(app) {
  ...
  const MONGO_URL = "mongodb://localhost:27017/employeedb09";
  ...
  MongoClient.connect(MONGO_URL, function(err, db) {
    ...
    var employees = db.collection(EMPLOYEE_COLLECTION, {strict:true}, function(err, collection) {
      if (err) {
        console.info(EMPLOYEE_COLLECTION + " collection doesn't exist. Creating it with sample data...");
        populateDB(db);
        console.log("Done initializing collection: " + EMPLOYEE_COLLECTION);
      }

      findById = function(req, res) {
        var id = parseInt(req.params.id);
        console.log('findById: ' + id + ":" + req.params);
...
var populateDB = function(db) {
  ...
  db.collection(EMPLOYEE_COLLECTION, function(err, collection) {
